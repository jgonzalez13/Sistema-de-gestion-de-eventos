...
export default function register() {
  ...
  {
    console.log('sw no registrado u.u)/')
  }
}

//offline ._.)/ espero funcione
function offline(){
  const header = document.querySelector('.Header'),
    metaTagThem = document.querySelector('meta[name=theme-color]')
  ...
  if('serviceWorker' in navigator && 'SyncManager' in window){
    function registerBDSync(){
      navigator.serviceWorker.ready
        .then(registration => {
          return registration.sync.register('correo')
            .then( () => console.log('Sincronización de Fondo Registrada'))
            .catch(err =>  console.log('No se segistró mi etiqueta'))
        })
        .catch( err => console.log('Fallo la Sincronización de Fondo ', err))
    }
    registerBDSync()
  }
}
